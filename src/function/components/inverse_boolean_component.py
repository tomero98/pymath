...
class InverseBooleanComponent(QWidget):
    ...
    label = 'Indicar sí existe inversa'
    ...
    def _validate_exercise(self, response: bool, pressed_button: QPushButton):
        ...
        is_answer_correct = has_function_inverse == response
        border_color = '#2F8C53' if is_answer_correct else 'red'

        yes_or_no_label = 'Sí' if not has_function_inverse else 'No'
        help_text = f'Correcto. {yes_or_no_label} se repiten imágenes para la gráfica dada.' if is_answer_correct \
            else f'Incorrecto. {yes_or_no_label} se repiten imágenes para la gráfica dada.'
    ...
    def _update_plot_with_error_data(self):
        ...
        color = (255, 0, 0)
        PlotFactory.update_plot(plot_widget=self._plot_widget, functions_to_update=functions_to_update,
                                help_points=points_to_update, show_ends=False, rgb_tuple=color)
